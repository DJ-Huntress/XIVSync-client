...
internal class JoinSyncshellUI : WindowMediatorSubscriberBase
{
	...
	private GroupJoinInfoDto? _groupJoinInfo;

	private DefaultPermissionsDto _ownPermissions;

	private string _previousPassword = string.Empty;

	private string _syncshellPassword = string.Empty;

	public JoinSyncshellUI(ILogger<JoinSyncshellUI> logger, MareMediator mediator, UiSharedService uiSharedService, ApiController apiController, PerformanceCollectorService performanceCollectorService)
		: base(logger, mediator, "Join existing Syncshell###XIVSyncJoinSyncshell", performanceCollectorService)
	{
		...
		base.SizeConstraints = new WindowSizeConstraints
		{
			...
			MaximumSize = new Vector2(700f, 400f)
		};
		base.Mediator.Subscribe<DisconnectedMessage>(this, delegate
		{
			base.IsOpen = false;
		});
		base.Flags = ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoCollapse;
	}
	...
	protected override void DrawInternal()
	{
		...
		if (_groupJoinInfo.GroupPermissions.IsPreferDisableSounds() != _ownPermissions.DisableGroupSounds || _groupJoinInfo.GroupPermissions.IsPreferDisableVFX() != _ownPermissions.DisableGroupVFX || _groupJoinInfo.GroupPermissions.IsPreferDisableAnimations() != _ownPermissions.DisableGroupAnimations)
		{
			ImGuiHelpers.ScaledDummy(2f);
			...
			if (_groupJoinInfo.GroupPermissions.IsPreferDisableSounds() != _ownPermissions.DisableGroupSounds)
			{
				...
				_uiSharedService.BooleanToColoredIcon(!_groupJoinInfo.GroupPermissions.IsPreferDisableSounds());
				ImGui.SameLine();
				using (ImRaii.PushId("suggestedSounds"))
				{
			...
			if (_groupJoinInfo.GroupPermissions.IsPreferDisableAnimations() != _ownPermissions.DisableGroupAnimations)
			{
				...
				_uiSharedService.BooleanToColoredIcon(!_groupJoinInfo.GroupPermissions.IsPreferDisableAnimations());
				ImGui.SameLine();
				using (ImRaii.PushId("suggestedAnims"))
				{
					if (_uiSharedService.IconTextButton(FontAwesomeIcon.ArrowRight, "Apply suggested"))
					{
			...
			if (_groupJoinInfo.GroupPermissions.IsPreferDisableVFX() != _ownPermissions.DisableGroupVFX)
			{
				...
				_uiSharedService.BooleanToColoredIcon(!_groupJoinInfo.GroupPermissions.IsPreferDisableVFX());
				ImGui.SameLine();
				using (ImRaii.PushId("suggestedVfx"))
				{
					if (_uiSharedService.IconTextButton(FontAwesomeIcon.ArrowRight, "Apply suggested"))
					{
		...
		if (_uiSharedService.IconTextButton(FontAwesomeIcon.Plus, "Finalize and join " + _groupJoinInfo.GroupAliasOrGID))
		{
			...
			_apiController.GroupJoinFinalize(new GroupJoinDto(_groupJoinInfo.Group, _previousPassword, joinPermissions));
			base.IsOpen = false;
		}
	}
}
