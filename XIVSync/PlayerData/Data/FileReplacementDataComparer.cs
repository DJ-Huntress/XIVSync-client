...
{
	private static readonly FileReplacementDataComparer _instance = new FileReplacementDataComparer();
	...
	public bool Equals(FileReplacementData? x, FileReplacementData? y)
	{
		...
		if (x.Hash.Equals(y.Hash) && CompareHashSets(x.GamePaths.ToHashSet<string>(StringComparer.Ordinal), y.GamePaths.ToHashSet<string>(StringComparer.Ordinal)))
		{
			return string.Equals(x.FileSwapPath, y.FileSwapPath, StringComparison.Ordinal);
		}
		return false;
	}
